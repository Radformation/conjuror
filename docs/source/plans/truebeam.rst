...
The ``MLCSpeed`` procedure adds two dynamic beams to test MLC Speed constancy
using a sequence of rectangular ROIs across the field.

- **Reference**: reference delivery with no modulation.
- **Modulated**: modulated delivery intended to achieve different MLC speeds per ROI.
...
.. code-block:: python
    ...
    import pydicom
    from conjuror.plans.plan_generator import PlanGenerator
    from conjuror.plans.truebeam import MLCSpeed

    base_plan = pydicom.dcmread(r"C:\path\to\base_plan.dcm")
    generator = PlanGenerator(base_plan, plan_name="MLC Speed", plan_label="MLCSpeed")

    procedure = MLCSpeed(
        speeds=(5, 10, 15, 20),  # mm/sec (one ROI per value)
        roi_size=25,
        desired_mu=50,
        y1=-100,
        y2=100,
    ...
    generator.add_procedure(procedure)
    generator.to_file("mlc_speed_plan.dcm")
...
Modulation is done through “sacrifices”, or “throws”, of MLC movement. Given that
(for Varian at least) all axes move as fast as they can, the beam will not use a
slower MLC speed unless something else is slowing it down. In Conjuror, the axes are
slowed down by moving the first and last MLC pair by a certain distance. Given a max
leaf speed, the movement will take a known amount of time. This time is then used to
calculate the MLC speed. By constraining the desired MU of a given ROI with the
sacrificial movements, the MLC speed can be modulated to a target value.
...
.. grid:: 2
    ...
    .. grid-item::
        ...
        .. plotly::
            ...
            from conjuror.plans.truebeam import MLCSpeed, TrueBeamMachine

            procedure = MLCSpeed(speeds=(5, 10, 15, 20), y1=-100, y2=100)
            machine = TrueBeamMachine(mlc_is_hd=False)
            procedure.compute(machine)
...
You can adjust the ROIs to test (``speeds``, ``roi_size``), the jaw geometry
(``y1``, ``y2``, ``jaw_padding``), and delivery/modulation parameters such as
``desired_mu``, ``default_dose_rate``, and ``max_sacrificial_move``. Standard
beam parameters (energy, fluence mode, gantry/collimator angles, couch positions)
are also available.
